...
### `qr_scans`
Scans and scan events. Exactly one of `qr_code_id`, `unified_qr_id`, `standard_link_id` is set.
...
### `qr_comments`
Works for both legacy and unified QRs.
...
- **Legacy**: `qr_codes`, `standard_links` (Firestore `qrCodes/`, `standardLinks/`)
