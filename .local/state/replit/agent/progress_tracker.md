...
[x] 25. Phase 4 — Server Hardening & Ecosystem Foundation:
        - server/lib/firebase-admin.ts: single initialisation of Firebase Admin SDK, exports
          getAdminApp(), getAdminDb(), getAdminAuth() — eliminates ad-hoc scattered init in
          routes/qr.ts and routes/payments.ts (both updated to import from here)
        - server/middleware/error-handler.ts: Express 5-compatible global error boundary,
          always returns structured { error, code, status } JSON — no unhandled rejections leak
        - server/middleware/validate.ts: validateBody(schema) Zod factory applied to POST routes;
          invalid payloads return 400 with per-field issues[]
        - server/middleware/cors.ts: origin whitelist driven by REPLIT_DEV_DOMAIN,
          REPLIT_DOMAINS, EXPO_PUBLIC_DOMAIN env vars; Authorization header now allowed
        - server/middleware/request-logger.ts: [METHOD /path STATUS Nms] per-route timing,
          dev-only, no-op in production
        - server/health-check.ts: GET /health and GET /api/v1/health now return
          { status, version, uptime, timestamp } — deployment readiness probes
        - server/routes/business.ts: POST /api/v1/business/register — verifies Firebase ID
          token, Zod-validates displayName, writes businessAccounts/{uid} with plan:"free"
        - server/routes/index.ts: business router wired under /api/v1/business
        - server/index.ts: replaced three inline setup* functions with the new middleware
          imports; dead setupCors / setupRequestLogging / setupErrorHandler functions removed
[x] 29. Navigation back-stack corruption fixes:
        - features/settings/SettingsScreen.tsx — handleMainBack:
          Was: safePush("/(tabs)/profile") / safePush("/(tabs)/history") based on `from` param.
          Problem: router.push to go "back" duplicates screens on the native stack.
          Stack before: Profile → Settings
          After wrong push: Profile → Settings → Profile (Android back now goes wrong way)
          Fix: replaced with safeBack() — pops Settings off the stack, returning naturally
          to whichever screen pushed it. No params needed to track "where we came from".
        - features/settings/SettingsScreen.tsx — handleSubSectionBack:
          Was: setSection("main") OR safePush("/(tabs)/profile") when fromProfile="1"
          Problem: sub-sections are state changes (same URL), nothing on native stack.
          The safePush added a new Profile screen to the stack instead of unwinding.
          Fix: always setSection("main") — state reset only, never a route push.
        - features/settings/SettingsScreen.tsx — params cleanup:
          Removed unused fromProfile and from from useLocalSearchParams type — they were
          only consumed by the now-fixed back handlers. initialSection retained.
        - Imported safeBack from @/shared/utils/navigation into SettingsScreen.
[x] 28. Route warning + edge-to-edge NavigationBar cleanup:
        - app/_layout.tsx: removed <Stack.Screen name="settings" /> — settings lives at
          app/(tabs)/settings.tsx (tab screen), not app/settings.tsx; stale Stack entry
          caused the "No route named settings exists" warning on every app start
        - app/_layout.tsx: removed setPositionAsync + setBackgroundColorAsync +
          setBorderColorAsync calls; removed colors.surfaceBorder from useEffect deps
        - shared/utils/use-android-nav-bar.ts: removed setPositionAsync +
          setBackgroundColorAsync from useAndroidNavBar + useAndroidNavBarScreen; unused
          openColor/restoreColor params stripped from useAndroidNavBar since color-setting
          is a no-op on edge-to-edge builds; only setButtonStyleAsync retained
        - shared/components/ui/BottomSheet.tsx: removed setPositionAsync("absolute") +
          setBackgroundColorAsync("transparent"); deps array updated (removed empty [])
        - features/settings/SettingsScreen.tsx: removed setBackgroundColorAsync call;
          deps array tightened ([colors.background, colors.isDark] → [colors.isDark])
        - lib/utils/use-android-nav-bar.ts: synced with updated shared version
        Root cause: targetSdkVersion 35 + newArchEnabled enforces Android edge-to-edge
        by default. Only setButtonStyleAsync (icon tint) is respected on API 35+;
        setPositionAsync / setBackgroundColorAsync / setBorderColorAsync are silently
        ignored and generate console warnings on every call.
[x] 27. Navigation & tab performance optimisation:
        - app/(tabs)/_layout.tsx:
          · Extracted IosTabBarBackground + AndroidTabBarBackground as React.memo module-level
            components — BlurView / View no longer recreated on every tab-bar render
          · Wrapped ScanTabButton in React.memo — LinearGradient (GPU-expensive) now stable
          · Moved ScanTabButton onPress into useCallback — no new function reference each render
          · Extracted HomeIcon, GeneratorIcon, HistoryIcon, ProfileIcon as React.memo components
          · Replaced inline tabBarIcon closures with module-level stable render functions
            (renderHomeIcon etc.) — tab bar no longer creates new arrow functions each paint
          · Wrapped screenOptions in useMemo — object rebuilt only when colors/insets change
          · Wrapped tabBarBackground in useCallback — stable reference for screenOptions memo
        - features/profile/ProfileScreen.tsx:
          · Wrapped StatCell in React.memo — had useSharedValue/useAnimatedStyle internals;
            previously new shared values were created on every parent re-render
          · Wrapped QrSkeletonCard in React.memo — prevents unnecessary animation re-triggers
          · Extracted DonationCard as React.memo with stable useCallback onPress — LinearGradient
            no longer recreated when parent profile state changes
          · Moved all FadeInDown/FadeIn animation presets to module-level constants
            (ENTER_TOP_BAR, ENTER_AVATAR_SEC, STAT_CELL_ENTER[], etc.) — Reanimated descriptor
            objects created once at module load, not on every render cycle
          · Replaced inline QR_SKELETON_INDICES [0,1,2] literal with module-level constant
        - shared/components/ui/SectionHeader.tsx:
          · Wrapped in React.memo — renders as a FlashList header row (HistoryScreen), was
            previously recreated on every list rerender cycle
[x] 26. My QR Code component cleanup:
        - Deleted 13 dead root-level duplicate files from features/my-qr/components/:
            GuardDestinationCard, QrContentInfoCard, QrHeroCard, QrStatsRow,
            StandardLinkCard, StaticContentEditor (→ duplicated in cards/)
            ConfirmActionModal, CustomColorModal, DeactivateModal, FollowersModal (→ modals/)
            DesignPanel, QrSettingsPanel (→ panels/)
            CommentRow (dead — OwnerCommentRow in comments/ is the canonical version)
        - MyQrDetailScreen.tsx already imported from subdirectories — zero import changes needed
        - Added barrel index.ts to each subdirectory for clean future imports:
