...
- **`apply_canvas_actions`** -- Create, update, delete, move, resize, reorder, align, distribute shapes, or draw with the pen tool.
...
Pen-created shapes currently come back from `get_canvas_state` as `shapeType: "draw"`. This tool does not yet support `draw` updates, so to change a pen stroke use `move` or `resize` when possible, or delete and recreate it with a new `pen` action.
...
Draw a freeform line or closed shape from a sequence of points.

```json
{
  "type": "pen",
  "shapeId": "arrow-sketch",
...
- `points` -- **Required.** Ordered list of `{x, y}` coordinates.
- `style` -- `"smooth"` (curved interpolation) or `"straight"` (line segments). Set it explicitly when the stroke style matters.
- `closed` -- If `true`, the path is closed and filled with `fill`.
- `color` -- Stroke color (same palette as other shapes).
- `fill` -- Fill style for closed paths.
