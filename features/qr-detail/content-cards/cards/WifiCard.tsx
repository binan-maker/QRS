    ...
    <View style={[styles.card, { backgroundColor: colors.surface, borderColor: accentColor + "45" }]}>
      ...
      <InfoGrid accentColor={accentColor} colors={colors} isDark={isDark}>
        ...
        {wifi.password ? (
          <>
            <Divider colors={colors} />
            <InfoRow label="Password" value={wifi.password} icon="key-outline" accentColor={accentColor} colors={colors} selectable />
          </>
        ) : null}
        {wifi.hidden ? (
          <>
            <Divider colors={colors} />
            <InfoRow label="Hidden" value="Yes" icon="eye-off-outline" accentColor={accentColor} colors={colors} />
          </>
        ) : null}
      </InfoGrid>
