  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.surface, borderColor: accentColor + "45" }]}>
      ...
      <CardHeader
        icon="key-outline"
        gradient={GRADIENT}
        title="Authenticator Key"
        subtitle={otp.issuer || otp.account}
        content={content}
        colors={colors}
      />
      ...
      <View style={[styles.warningRow, { backgroundColor: isDark ? "#2D1A00" : "#FFFBEB", borderColor: "#F59E0B40" }]}>
        ...
        <Text style={[styles.warningText, { color: "#B45309" }]}>
          Only add to your authenticator app if you recognize this account.
        </Text>
      </View>
      ...
      >
        ...
        <Text style={[styles.copyBtnText, { color: accentColor }]}>
          {copied ? "Copied Key!" : "Copy Key URI"}
        </Text>
      </Pressable>
...
const styles = StyleSheet.create({
  card: {
    ...
    borderWidth: 1, overflow: "hidden", gap: 12,
  },
  infoGrid: {
  ...
  warningText: {
    fontSize: 11, fontFamily: "Inter_400Regular", flex: 1, lineHeight: 16,
  },
  copyBtn: {
