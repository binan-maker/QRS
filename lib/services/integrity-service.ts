...
// SECURITY FIX P0: Hourly + per-QR rate limits on report submissions
// Prevents financial damage (Firestore writes) and trust-score manipulation.
const HOURLY_REPORT_WINDOW_MS = 3_600_000; // 1 hour
const HOURLY_REPORT_LIMIT_PER_USER = 10;   // 10 reports / user / hour
const HOURLY_REPORT_LIMIT_PER_QR = 3;      // 3 reports / QR / user / hour
...
export async function checkReportEligibility(
  ...
  // SECURITY FIX P0: Enforce rate limits (only for new reports, not toggles)
  if (!isChangingReport) {
    try {
      const userData = await db.get(["users", userId]);

      // 1) Daily limit (per-tier)
      ...
      // 2) Hourly limit (10/hour, all tiers — abuse prevention)
      ...
      // 3) Per-QR limit (3/QR/user/hour — prevents dogpiling a single QR)
      try {
      ...
      // Re-throw user-facing rate limit errors; swallow infrastructure errors
...
export async function recordReport(userId: string, qrId: string): Promise<void> {
  try {
    ...
    // Per-(user, QR) counter for the 3/QR/user/hour cap
    try {
      const perQr = await db.get(["users", userId, "reportLog", qrId]);
      const perQrWindowStart = tsToMs(perQr?.windowStart);
      const perQrCount = perQr?.count || 0;
      if (isWithinMs(perQrWindowStart, HOURLY_REPORT_WINDOW_MS)) {
        ...
        await db.set(["users", userId, "reportLog", qrId], {
          count: 1,
          windowStart: db.timestamp(),
          updatedAt: db.timestamp(),
        });
      }
    } catch {}

    // Velocity tracking on the QR doc (existing collusion-detection input)
