...
const PROFANITY_PATTERNS: {
  ...
  {
    pattern: /\b(nigga|nigger)\b/gi,
    severity: "severe",
    category: "hate_speech",
  },
  {
    pattern: /\b(fag|faggot|queer)\b/gi,
    severity: "severe",
    category: "hate_speech",
  },
  {
    pattern: /\b(racist|slant-eye)\b/gi,
    severity: "severe",
    category: "hate_speech",
  },
  ...
  // Severe - Threats
  {
    pattern: /\b(kill\s+you|murder|death\s+to|die\b)/gi,
    severity: "severe",
    category: "threats",
  },
  ...
  {
    pattern: /\b(fuck|fucker|fucking|fucked)\b/gi,
    severity: "moderate",
    category: "profanity",
  },
  {
    pattern: /\b(shit|shitty)\b/gi,
    severity: "moderate",
    category: "profanity",
  },
  {
    pattern: /\b(bitch|bitches)\b/gi,
    severity: "moderate",
    category: "profanity",
  },
  {
    pattern: /\b(damn|dammit)\b/gi,
    severity: "moderate",
    category: "profanity",
  },
  ...
  {
    pattern: /\b(stupid|idiot|dumb|asshole|bastard)\b/gi,
    severity: "mild",
    category: "offensive",
  },
  ...
  // Sexual content
  {
    pattern: /\b(porn|sex|xxx|nude|naked)\b/gi,
    severity: "moderate",
    category: "sexual",
  },
  ...
  {
    pattern: /\b(click\s+here|free\s+money|win\s+now|claim\s+prize)\b/gi,
    severity: "mild",
    category: "spam",
  },
];
...
const HINDI_PROFANITY: {
  ...
  {
    pattern: /\b(गांड|चूत|लौड़ा|बहनचोद|मादरचोद)\b/gi,
    severity: "severe",
    category: "profanity",
  },
  {
    pattern: /\b(bc|mc|bsdk|chutiya)\b/gi,
    severity: "moderate",
    category: "profanity",
  },
];
...
export function checkProfanity(text: string): ProfanityCheckResult {
  ...
  for (const { pattern, severity, category } of allPatterns) {
    ...
    if (matches && matches.length > 0) {
      ...
      else if (severity === "moderate" && maxSeverity !== "severe")
        maxSeverity = "moderate";
      else if (severity === "mild" && maxSeverity === "none")
        maxSeverity = "mild";
...
=======
  const { validateQrContent } = require('../analysis/qr-validator') as {
>>>>>>> 04a70bd482f9cd8e558cbefbec649cb6cfb9b39b
  ...
  // Profanity screen — only applied to plain-text payloads, never to URLs / UPI / EMV
  // (we don't want to block a legitimate URL because of a substring match).
  const trimmed = (content || "").trim();
  const isStructured =
    /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(trimmed) || /^00020[12]/.test(trimmed);
  if (!isStructured) {
