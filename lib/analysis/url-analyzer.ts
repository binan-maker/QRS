...
// These are strictly controlled by Indian and global authorities.
// They CANNOT be freely registered — only verified entities get them.
// Flagging a brand impersonation on these is almost always a false positive.
const REGULATED_SLDS = new Set([
  // India — Government & Institutional
  "gov.in",   // Government of India ministries / departments
  "nic.in",   // National Informatics Centre (govt IT)
  ...
  "net.in",   // ISPs regulated by DoT
  // India — Financial (RBI / SEBI regulated)
  "bank.in",  // RBI-licensed scheduled banks ONLY
  "fin.in",   // RBI-designated financial institutions
  "co.in",    // Indian companies (Registrar of Companies)
  "org.in",   // Indian non-profits
  // Global institutional
  "gov",      // US Federal Government
  "mil",      // US Military
  "edu",      // Accredited US universities
  "int",      // International treaty organisations
]);

/**
 * Returns true if the hostname ends with a tightly regulated SLD
 * that cannot be freely registered by members of the public.
 */
function isRegulatedDomain(hostname: string): { regulated: boolean; sld: string } {
  for (const sld of REGULATED_SLDS) {
    if (hostname === sld || hostname.endsWith("." + sld)) {
      return { regulated: true, sld };
    }
  }
  return { regulated: false, sld: "" };
}
