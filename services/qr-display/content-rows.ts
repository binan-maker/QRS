...
export function parseQrContentDetails(item: any): ContentDetailRow[] {
  ...
  switch (contentType) {
    ...
    case "calendar": {
      ...
      if (desc)
        rows.push({ label: "Description", value: desc.length > 60 ? desc.slice(0, 60) + "…" : desc, icon: "document-text-outline" });
      return rows;
    }
    case "contact": {
    ...
    case "crypto": {
      ...
      const rows: ContentDetailRow[] = [{ label: "Coin", value: coin.charAt(0).toUpperCase() + coin.slice(1), icon: "logo-bitcoin" }];
    ...
    case "menucatalogue": {
      ...
      try {
        const u = new URL(src.startsWith("http") ? src : `https://${src}`);
        return [{ label: "Menu / Catalogue", value: u.hostname.replace(/^www\./, ""), icon: "list-outline" }];
      } catch {}
      return [];
    }
    case "donation": {
    ...
    case "spotify": {
      try {
        ...
        if (parts.length >= 2)
          return [{ label: parts[0].charAt(0).toUpperCase() + parts[0].slice(1), value: parts[1].replace(/-/g, " "), icon: "musical-notes-outline" }];
      } catch {}
      return [];
    }
    case "discord": {
